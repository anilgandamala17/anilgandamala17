            ...
            if (stateData?.competitiveQuestion && topicId?.startsWith('competitive-')) {
                ...
                const teachingSteps = [
                    ...
                    },
                    {
                        ...
                        content: Array.isArray(question.options) && question.options.length > 0
                            ? `### All Options:\n${question.options.map((opt: string, idx: number) => `**${String.fromCharCode(65 + idx)}.** ${opt}`).join('\n')}\n\n---\n\n### ✅ Correct Answer: **${String.fromCharCode(65 + question.correctAnswer)}. ${question.options[question.correctAnswer] ?? ''}**`
                            : `### ✅ Correct Answer:\n${question.correctAnswer ?? question.explanation ?? 'See solution above.'}`,
                        spokenContent: Array.isArray(question.options) && question.options.length > 0
                            ? `Now let us verify the answer. The correct answer is Option ${String.fromCharCode(65 + question.correctAnswer)}: ${question.options[question.correctAnswer] ?? ''}.`
                            : `Now let us verify the final answer.`,
                        visualType: 'text' as const,
                        visualDomain: 'competitive',
                        durationSeconds: 45,
                        completed: false,
