...
export default function CurriculumVideoPlayer({
  ...
  const useNativeEnglishAudio = useCallback(() => {
  ...
  useEffect(() => {
    ...
    let translateRequestedFor: TtsLanguageCode | null = null
    ...
    const applyFromLanguages = async (languages: VideoLanguageDto[]): Promise<'ready' | 'wait' | 'missing'> => {
      ...
      // One translate request per language selection; HEAD probes on poll pick up
      // files dropped later without re-queuing forever.
      if (translateRequestedFor !== ttsLanguage) {
        translateRequestedFor = ttsLanguage
    ...
    // Keep probing while the dub is pending/queued/missing so a newly added MP3
    // is picked up without remounting or seeking the MP4 back to 0.
    const poll = window.setInterval(() => {
      if (cancelled || isEnglishTtsLanguage(ttsLanguage) || usingDubRef.current) return
      void refresh().then((result) => {
        if (result === 'ready') window.clearInterval(poll)
      })
    }, 3000)
  ...
  }, [ttsLanguage, resource.id, applyCaptions, useNativeEnglishAudio, englishTrack?.captions, resource.captionsSrc])
