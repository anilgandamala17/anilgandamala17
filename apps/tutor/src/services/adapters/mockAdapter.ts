...
function withLiveAudioTracks(video: CurriculumVideoResource): CurriculumVideoResource {
  const overlay = readTrackOverlay(video.id)
  return {
    ...video,
    video_audio_tracks: video.video_audio_tracks.map((track) => {
      if (isEnglishTtsLanguage(track.language)) return { ...track, status: 'ready' }
      const overlayStatus = overlay[track.language]
      return overlayStatus ? { ...track, status: overlayStatus } : track
    }),
  }
}
...
export const mockAdapter = {
  ...
  async listTopicVideoResources(topicId: string): Promise<CurriculumVideoResource[]> {
    await demoDelay(80)
    return listPublishedVideosForTopic(topicId).map(withLiveAudioTracks)
  },
   ...
   * NOW: ready if dub files exist; else { status: "queued" } then flip after a short delay.
